...
FirebaseDynamicLinks.instance.onLink.listen((PendingDynamicLinkData? dynamicLink) async {
    ...
    print('CONNECTE'); 
    print(deepLink);
    if (deepLink != null && deepLink.path == '/sam') {
      print("Lien dynamique détecté: /sam");

      // Récupérer les informations de l'utilisateur via le ProfileController
      final String userID = Provider.of<ProfileController>(navigatorKey.currentContext!, listen: false).userID;

      // Accéder à l'OrderController pour récupérer les informations de la commande
      final orderController = Provider.of<OrderController>(navigatorKey.currentContext!, listen: false);

      // Initier la récupération de la commande avec l'ID utilisateur
      await orderController.getOrderList(1, 'ongoing');

      // Parcourir la liste des commandes pour trouver celle de l'utilisateur
      final Orders? orderModel = orderController.orderModel?.orders?.firstWhere(
        (order) => order.customerId == int.parse(userID),
      );

      if (orderModel != null) {
        // Naviguer vers TrackingResultScreen avec l'orderID
          Navigator.push(
        navigatorKey.currentContext!,
        MaterialPageRoute(
            builder: (context) => TrackingResultScreen(orderID: orderModel.id.toString()),
        ),
      );
      } else {
    ...
    print("Erreur lors de la récupération du lien dynamique: $error");
  });
